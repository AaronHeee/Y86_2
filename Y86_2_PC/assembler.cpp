...
bool Assembler::compileFile(istream& in, istream&in_cp,char *buffer ,ostream &out)
{
    resetAssembler();
    Adr(in_cp);
    // fo(i,0,TagLength)
...
//         out <<"0x000: "<< string(SPACELENGTH -7 ,' ')<< "| "<< line<<endl;
    ...
    while(in.getline(line,LENGTH))
    {
        ...
        if(ex(SHALT))   buffer[Mem_index++] = CHALT;
        else if(ex(SNOP))   buffer[Mem_index++] = CNOP;
        else if(ex(SRET))   buffer[Mem_index++] = CRET;
        ...
        || ex(SSUBL) || ex(SANDL) || ex(SXORL) )
        {
            ...
            fo(i,0,7)
                if(ex(SCMOV[i]))
                {
                    buffer[Mem_index++] = CRRMOVL + i;
                    break;
                }
            fo(i,0,4)
                if(ex(SOPL[i]))
                {
                    buffer[Mem_index++] = COPL + i;
                }
            char a_b = twoReg(line);
            buffer[Mem_index++] = a_b;
        }
        else if(ex(SIRMOVL) || ex(SRMMOVL) || ex(SMRMOVL))
        {
            if(ex(SIRMOVL))
            {
                buffer[Mem_index++] = CIRMOVL;
            ...
            {
                if(ex(SRMMOVL))
                    buffer[Mem_index++] = CRMMOVL;
                else
                    buffer[Mem_index++] = CMRMOVL;
            ...
            buffer[Mem_index++] = v_1;
            buffer[Mem_index++] = v_2;
            buffer[Mem_index++] = v_3;
            buffer[Mem_index++] = v_4;
        }
        else if(ex(SPUSHL) || ex(SPOPL))
        {
            if(ex(SPUSHL))  buffer[Mem_index++] = CPUSHL;
            else buffer[Mem_index++] = CPOPL;
        ...
        {
            if(ex(SCALL))
                buffer[Mem_index++] = CCALL;
            else
            {
                fo(i,0,7)
                    if(ex(SJXX[i]))
                    {
                        buffer[Mem_index++] = CJXX + i;
                        break;
                    }
            }
        ...
        else if(ex(SALIGN))
        {
            ...
            while(Mem_index % alignNum) buffer[Mem_index++] = 0x00;
        }
        ...
        if(ex(".pos"))
        {
            ...
            if(pos_flag == 1)
            {
                ...
                while(Mem_index != pre_index) buffer[Mem_index++] = 0x00;
            }
...
void Assembler::Y86Read(istream& in,char *buffer)
{
    char num;
    int index = 0;
    while(in >> num)
