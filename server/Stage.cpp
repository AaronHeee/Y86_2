...
void Stage::output(Info *info) {
    printf("\n");
    printf("Stat = %02x\n", info->stat);
    /* Write Back */
    printf("\n");
    printf("Write:");
    printf("   W_stat = %02x", info->W_stat);
    printf("  W_icode = %02x", info->W_icode);
    printf("  W_valE = %08x", info->W_valE);
    printf("  W_valM = %08x", info->W_valM);
    printf("  W_dstE = %02x", info->W_dstE);
    printf("  W_dstM = %02x", info->W_dstM);
    printf("\n");
    ...
    printf("\n");
    printf("Memory:");
    printf("  M_stat = %02x", info->M_stat);
    printf("  M_icode = %02x", info->M_icode);
    printf("  M_Cnd = %01x", info->M_Cnd);
    printf("  M_valE = %08x", info->M_valE);
    printf("  M_valA = %08x", info->M_valA);
    printf("  M_dstE = %02x", info->M_dstE);
    printf("  M_dstM = %02x", info->M_dstM);
    printf("\n");
    ...
    printf("\n");
    printf("       ");
    printf("  ZF = %01x  SF = %01x  OF = %01x",info->CC[0],info->CC[1],info->CC[2]);
    printf("\n");

    printf("\n");
    printf("Execute:");
    printf(" E_stat = %02x", info->E_stat);
    printf("  E_icode= %02x", info->E_icode);
    printf("  E_ifun = %02x", info->E_ifun);
    printf("  E_valC = %08x", info->E_valC);
    printf("  E_valA = %08x", info->E_valA);
    printf("  E_valB = %08x", info->E_valB);
    printf("\n       ");
    printf("  E_dstE = %02x", info->E_dstE);
    printf("  E_dstM = %02x", info->E_dstM);
    printf("  E_srcA = %02x", info->E_srcA);
    printf("  E_srcB = %02x", info->E_srcB);
    printf("\n");
    ...
    printf("\n");
    printf("Decode:");
    printf("  D_stat = %02x", info->D_stat);
    printf("  D_icode = %02x", info->D_icode);
    printf("  D_ifun = %02x", info->D_ifun);
    printf("  D_rA = %02x", info->D_rA);
    printf("  D_rB = %02x", info->D_rB);
    printf("  D_valC = %08x", info->D_valC);
    printf("  D_valP = %08x", info->D_valP);
    printf("\n");
    ...
    printf("\n");
    printf("Fetch:");
    printf("   F_predPC = %08x", info->F_predPC);
    printf("\n");
...
char* Stage::strc2char(Info *info) {
    int n = sizeof(*info);
    char* buf = new char[n];
...
Info* Stage::char2strc(char* buf) {
    Info* info = new Info;
    memcpy(info, buf, sizeof(*info));
    return info;
}

void Stage::mapping(int cycle) {

    printf("cycle = %d\n",cycle);

    info->F_predPC = F_predPC;

    info->D_stat  = D_stat;
    info->D_icode = D_icode;
    info->D_ifun  = D_ifun;
    info->D_rA    = D_rA;
    info->D_rB    = D_rB;
    info->D_valC  = D_valC;
    ...
    info->W_stat  = W_stat;
    info->W_icode = W_icode;
    info->W_valE  = W_valE;
    info->W_valM  = W_valM;
    info->W_dstE  = W_dstE;
    info->W_dstM  = W_dstM;

    info->F_stall = F_stall;
    info->D_stall = D_stall;
    info->W_stall = W_stall;
    ...
    for (int i = 0; i < M_MAX; i++)
        info->memory[i] = memory.mem[i];

    for (int i = 0; i < 8; i++)
        info->reg[i] = regFile.reg[i];
